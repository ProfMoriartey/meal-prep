...
// New Zod schema for updating a meal
export const updateMealFormSchema = z.object({
  id: z.number().int().positive({ message: "Meal ID is required." }),
  mealName: z.string().min(1, "Meal name is required."),
  ingredients: z.array(
    z.object({
      name: z.string().min(1, "Ingredient name is required."),
...
export type UpdateMealForm = z.infer<typeof updateMealFormSchema>;
